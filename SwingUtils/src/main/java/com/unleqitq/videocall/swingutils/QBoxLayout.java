...
public class QBoxLayout implements LayoutManager2, Serializable {
	...
	public QBoxLayout(Container target, int axis) {
	 ...
	 *               null if none
	 */
	QBoxLayout(Container target, int axis, PrintStream dbg) {
		this(target, axis);
		this.dbg = dbg;
	}
	...
	public void layoutContainer(Container target) {
		...
		int x = in.left;
		int y = in.top;
		int maxOtherSize = target.getWidth() - in.left - in.right;
		
		// flush changes to the container
		for (int i = 0; i < nChildren; i++) {
			Component c = target.getComponent(i);
			int x0 = x + c.getPreferredSize().width;
			if (x > maxOtherSize) {
				y += c.getPreferredSize().height;
				x = in.left;
				c.setBounds(x, y, (int) (c.getPreferredSize().getWidth() * 1.1),
						(int) (c.getPreferredSize().getHeight() * 1.1));
			}
			else {
				c.setBounds(x, y, (int) (c.getPreferredSize().getWidth() * 1.1),
						(int) (c.getPreferredSize().getHeight() * 1.1));
